...
| `autoFan` | optional | `{ "startPct": 60, "endPct": 42, "mode": "knee" }` — instead of per-point fan values, let the app schedule the fan from the **measured bean temperature**, declining from `startPct` to `endPct` (the fan speed it finishes at). `mode` selects the shape: `"knee"` (default — hold early, ease only slightly until 160 °C bean temp, then drop faster to the end speed), `"hold"` (full start speed until 160 °C, then straight-line down), `"linear"` (even decline from 100 °C to the end temp), `"steps"` (start speed until dry end, halfway until first crack, end speed after). `endPct` and `mode` are optional (defaults: 30 % below start / knee). Optional `midPct` (with `midTempC`, default 160) inserts a middle point: the fan runs straight lines from `startPct` to `midPct` at that bean temp, then to `endPct` — `midPct` may sit **above** both for a rise-then-fall "pyramid" airflow (e.g. `"startPct": 60, "midPct": 80, "midTempC": 160, "endPct": 42`); `mode` is ignored when `midPct` is set. When `autoFan` is present, per-point `fanPct` values are ignored during the run. Prefer this unless the user asks for explicit fan control. |
