...
| `autoFan` | optional | `{ "startPct": 60, "dropPct": 30, "mode": "knee" }` — instead of per-point fan values, let the app schedule the fan from the **measured bean temperature**, declining from `startPct` to a floor `dropPct` percent below it (60 with 30 → floor 42). `mode` selects the shape: `"knee"` (default — hold early, ease only slightly until 160 °C bean temp, then drop faster to the floor), `"hold"` (full start speed until 160 °C, then straight-line to the floor), `"linear"` (even decline from 100 °C to the end temp), `"steps"` (start speed until dry end, half the drop until first crack, floor after). `dropPct` and `mode` are optional (defaults 30 / knee). When `autoFan` is present, per-point `fanPct` values are ignored during the run. Prefer this unless the user asks for explicit fan control. |
