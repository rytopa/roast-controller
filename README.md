...
- **Green inventory** — track your green coffee stock: add beans by hand or **photograph the bag label** and OCR (Tesseract.js, in-browser) pre-fills the name, origin, and bag weight. Pick the bean in the Roast session panel and the batch's **green weight is deducted from stock automatically at Drop** (correcting the weight afterwards adjusts it; switching the bean moves it). Out-of-stock beans are flagged, and the inventory syncs with Cloud sync like profiles and logs (if you set up sync before this feature existed, re-paste the updated `sync-worker.js` into your Worker).
