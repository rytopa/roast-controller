...
- **Batch codes** — every roast gets a 3-character code (e.g. `#K7Q`) at Charge, shown next to the timer, on the roast summary and in the saved-roast list, so you can write it on the bag and find the roast again later. Codes skip look-alike characters and never repeat across your saved roasts.
- **Notes & stars** — a notes box in the Data panel follows the roast selected in the saved-roast list (or holds a draft for the roast in progress) and saves as you type; the ★ button marks an excellent roast so it stands out in the saved-roast list. At Charge, the selected inventory bean’s notes (including tasting/label notes) are copied into the new roast’s editable notes. They are saved at Drop and remain with that roast even if the inventory entry changes later; the batch picker previews what will be copied.
- **Safe endings** — when a running profile reaches its last point the heater switches off automatically (with an alert); pressing **Stop / Drop** on the Controls rail with a roast running ends it: the drop is marked, the roast is saved and recording stops (the finished curve stays on the chart for review until you press Charge for the next batch, start the between-batch cycle, or Clear chart), the heater goes to 0 and the fan returns to the speed the roast began with (the charge fan, or the profile's opening fan). With no roast running it is a plain emergency heater cut. The batch's green weight is deducted from the selected inventory bean at that moment.
...
- **Profile generator** — don't want to draw the curve by hand? Enter your **dry-end temp, first-crack temp and drop temp** plus **how long each phase should take**, and pick a shape: **Declining RoR** (starts high, eases down all roast — the classic), **Pyramid RoR** (climbs through drying to a peak at dry end, then declines), **Straight phases** (constant RoR per phase), **Soak start** (near-flat target for the first ~45 s, gentle on dense beans, then a declining curve), **Flat development** (declining until first crack, then the RoR holds steady through development — helps hit the drop time without stalling), **Step-down at FC** (sheds a chunk of RoR in the ~30 s before first crack — the anti-crash/flick move — then a calm flat development), **Boil-off plateau** (parks ~60–90 s at the boiling point so heater energy evaporates moisture at constant temp before Maillard), **FC exotherm bump** (a small deliberate RoR *rise* into first crack so the PID holds power through the FC steam crash and the real RoR comes out flat), **Maillard valley** (hot drying into a near-stall dwell just past dry end — buying time-at-temperature for Maillard chemistry — then a second push and decline; experimental) **Free-fall** (exponential RoR decay — the natural curve of a constant heater, minimal PID actuation) or **Sprint** (flat out to a chosen temp, 140 °C by default, by a time you set, then a slow steady dwell through the band from there to dry end — e.g. 100 → 140 °C in the first minute and the remaining three minutes of a 4-minute drying phase spent between 140 and 160 °C; the RoR deliberately picks up again into Maillard). Two modifiers combine with any shape: **⚡ Precharge** commands +8 °C above the curve for the first 40 s (full power into the cold wet beans, shorter turnaround), and **🍃 Coast** ends the command 3 °C short of the drop and lets stored drum heat carry the beans the rest of the way. The generator builds a curve that hits your phase targets at exactly those times, loads it into the designer graph (name, notes with per-minute deltas, and the DE/FC phase markers filled in), so you can flip between the shapes to compare, tweak points, then save.
- **Between-batch protocol** — one button between roasts: blasts the fan (heater off) to cool the machine to a set temp, then ramps the heater up gradually (never more than 1.5% per second, capped at 85%) until the drum reaches your charge temp at a low fan speed, holds it for 30 s, and lights up **DROP BEANS** (with a beep/vibration) when it's stable. The roaster's own PID is never engaged between batches — the app drives the heater open-loop so power only ever ramps, with no full-power surge at start-up or at the bean drop (the heater is parked on the soak's first step the instant the drop is detected).
- **DE / FC prediction** — Artisan-style **»DE** and **»FC** countdown tiles under the roast timer show how long until the bean temp reaches the Dry End and First Crack temperatures from the Phases row, with the predicted clock time (e.g. *»FC · at 7:40*), and the chart's dashed bean-temp projection now runs out to the predicted First Crack with *DE ~4:44* / *FC ~7:39* markers. The projection takes the slope of the last 45 s and bends it by the falling RoR of the last 60 s (an exponential decay towards a 1 °C/min floor — the way RoR really behaves — so a straight line does not arrive too early); a **Predict DE / FC** setting next to *RoR smooth* switches to a plain **linear** projection or **off**. Predictions start once the turning point is confirmed, are smoothed so they do not flicker, turn into the actual time once the event is reached, and with a background roast loaded the line under the tiles shows *DE vs BG +0:12*. Live only — nothing is stored with the roast.
- **Manual roast** — its own **🎛 Start manual roast** button (own panel above the between-batch one). One tap asks for the bean and green weight (the same dialog as the cycle), marks Charge — timer, chart recording and batch code start — sets the fan then the heater to the opening values (60 % / 60 % by default, remembered) with the PID off, and hands the roast to you on the Controls rail. No between-batch cycle, soak or profile is involved; Stop / Drop ends and saves the roast as usual. As in every mode, if DE has not been tagged by the time the bean temperature reaches 180 °C, it is tagged automatically at the moment BT crossed the DE temperature.
...
- **Charge soak** — a profile started at Charge begins open-loop: heater fixed at **30% until 0:30**, then **40% until 0:45** on the roast clock, after which the PID takes over and follows the curve. A **profile clock** option next to the soak settings chooses where the curve's 0:00 sits: **runs from charge** (the PID joins the curve part-way in, at 0:45 by default) or **starts after the soak** (the curve's 0:00 is the end of the soak, so the PID starts at the first point and the whole curve plays after the soak — the roast runs that much longer, and the plan on the chart shifts to match). The soak belongs to the between-batch cycle's auto-charge: pressing **▶ Run profile** by hand skips it, stops any running cycle and engages the PID on the curve immediately, from whatever state the machine is in.
...
- **Live guidance** — a live Dry/Maillard/Dev phase bar, an automatic **Dry End tag** (whatever is running the roast, an untagged DE is stamped at the DE-temp crossing once BT reaches 180 °C), a dotted **BT projection** (60 s look-ahead at current RoR), an **FC countdown**, and automatic **RoR crash / flick warnings**. The RoR trace has an adjustable **window** (responsiveness) and **smoothing** (a moving average that cleans the jaggedness from the roaster's ~1 °C telemetry steps).
...
- **Green inventory** — track your green coffee stock: add beans by hand or **photograph the bag/spec label** to auto-fill the name, origin, bag weight, and a tidy spec summary (variety, region, grade, altitude, density, moisture, process, suggested roast, cupping notes). Two label readers: on-device OCR (Tesseract.js, **English + Chinese**) by default, or — if you paste your own API key in the panel — **AI label reading** that sends the photo to a vision model for far better results on messy, dense, or multi-language labels. Pick your provider: **Claude (Anthropic)** or **Gemini (Google)** (which has a free tier). By default the key stays in your browser only (never cloud-synced) and the photo goes straight to your own account. For shared or public use, set a **Proxy Worker URL** instead — deploy [`ai-proxy-worker.js`](ai-proxy-worker.js) as your own Cloudflare Worker (holding the key as a secret), and the browser calls the Worker rather than the provider, so no key ever lives on the device. Pick the bean in the Roast session panel and the batch's **green weight is deducted from stock automatically at Drop** (correcting the weight afterwards adjusts it; switching the bean moves it). Out-of-stock beans are flagged, and the inventory syncs with Cloud sync like profiles and logs (if you set up sync before this feature existed, re-paste the updated `sync-worker.js` into your Worker). A **batch planner** splits a bean's remaining weight into equal batches that fit your roaster's min–max size (e.g. 850 g at 100–200 g → **5 × 170 g**, no awkward leftover); each bean card shows the suggested split at a glance. In the Roast session panel, selecting a bean shows a **Use *N* g** button that loads one balanced batch straight into Green g — roast it, and Drop deducts it, with the suggestion rebalancing for the next batch.
- **Link-loss protocol** — if Bluetooth drops mid-roast the roast and any running profile / cycle are kept and paused, the app alarms and reconnects to the same roaster by itself (every 3 s for up to 3 min), then re-sends the current heater command and resumes on the same clock. The roaster holds its last setting meanwhile, so the banner tells you to cut the heat at the machine if it is not back within a minute.
- **Manual controls** — fan, direct heater power, and PID on/off in an advanced section.
- **Control rail** — a stock-app style column beside the chart, and the place you drive the roast from: Target temp and PID tiles, Heater −/+ and Fan −/+ steppers in 1 % steps, with a tap on the value itself opening −5 / +5 buttons (heater ± takes manual control: PID off, running profile stopped; the Heater and Fan tiles are themselves OFF buttons for that output), a green event-tagging button that advances DE → FC, and a red Stop / Drop.
...
- `autoFan` — optional `{ "startPct": 60, "endPct": 42, "mode": "knee" }`: bean-temp-driven fan schedule declining from the start speed to the end speed, shaped by `mode` (`knee` gentle-then-faster around 160 °C · `hold` flat then straight down · `linear` even decline · `steps` at the DE/FC temps). Optional `midPct`/`midTempC` add a middle point — straight lines start → mid → end, allowing a rise-then-fall "pyramid" (mid above both); `mode` is ignored then. Overrides per-point `fanPct` while running. (Legacy `dropPct` is still accepted.)
