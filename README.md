...
- **Profile generator** — don't want to draw the curve by hand? Enter your **dry-end temp, first-crack temp and drop temp** plus **how long each phase should take**, and pick a shape: **Declining RoR** (starts high, eases down all roast — the classic), **Pyramid RoR** (climbs through drying to a peak at dry end, then declines), **Straight phases** (constant RoR per phase), **Soak start** (near-flat target for the first ~45 s, gentle on dense beans, then a declining curve), **Flat development** (declining until first crack, then the RoR holds steady through development — helps hit the drop time without stalling) or **Step-down at FC** (sheds a chunk of RoR in the ~30 s before first crack — the anti-crash/flick move — then a calm flat development). The generator builds a curve that hits your phase targets at exactly those times, loads it into the designer graph (name, notes with per-minute deltas, and the DE/FC phase markers filled in), so you can flip between the shapes to compare, tweak points, then save.
