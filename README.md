...
- **Profile generator** — don't want to draw the curve by hand? Enter your **dry-end temp, first-crack temp and drop temp** plus **how long each phase should take**, and pick a shape: **Declining RoR** (starts high, eases down all roast — the classic), **Pyramid RoR** (climbs through drying to a peak at dry end, then declines) or **Straight phases** (constant RoR per phase). The generator builds a curve that hits your phase targets at exactly those times, loads it into the designer graph (name, notes with per-minute deltas, and the DE/FC phase markers filled in), so you can flip between the shapes to compare, tweak points, then save.
