...
- **FC power hold** — optional **⚡ Hold power at FC** tick under the chart: tagging First Crack fixes the heater at its **last-30 s average %** and keeps the fan where it is (the automated fan is paused) for a configurable number of seconds, then hands back to the PID automatically (0 = hold until Drop); a **▶ Resume PID** button releases it early. Re-tagging FC re-averages and restarts the clock. Works with profile, plain PID and ET-track roasts.
