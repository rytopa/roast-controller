...
- **Charge soak** — a profile or ET-track run started at Charge begins open-loop: heater fixed at **30% until 0:30**, then **40% until 0:45** on the roast clock, after which the PID (or ET-track loop) takes over and follows the curve. (A run started without a recent Charge — e.g. restarted mid-roast — engages its loop immediately.)
