...
- **Batch size / Roasted weight / Weight loss** sit above the notes for the current roast. Batch size is the green weight (filled in from the between-batch dialog, editable here); type the **roasted weight** once the beans have cooled and the **weight loss %** appears and is saved with the roast — you can also do this later after loading the roast from the list.
